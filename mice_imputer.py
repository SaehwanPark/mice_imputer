...
class MiceImputer:
  ...
  def _configure_imputation(
    ...
    for tgt in self.columns_to_impute_:
      ...
      pr = [
        c
        for c in pr
        if (1.0 - df[c].is_null().mean()) >= (1 - self.max_predictor_missing_rate)
      ]
      mapping[tgt] = pr
  ...
  def _fit_iteration(self, data: pl.DataFrame, iteration: int) -> pl.DataFrame:
    ...
    for target in self.columns_to_impute_:
      ...
      predictors = [c for c in self.predictor_mapping_[target] if c in out.columns]
      ...
      if cfg.col_type == "continuous":
        ...
        if cfg.col_type == "ordinal" and cfg.order is not None:
          ...
          y_obs = np.array(
            [cat_to_idx.get(v, np.nan) for v in y_obs_series.to_list() if v is not None]
          )
      ...
      if len(y_obs) < 5 or X_obs.shape[0] < 5:
        warnings.warn(f"Insufficient training data for {target}; using simple fill.")
        if cfg.col_type == "continuous":
      ...
      if task == "regression":
        ...
        if cfg.pmm:
          donors = _pmm_sample(y_pred_mis, y_pred_obs, y_obs, k=cfg.pmm_k, rng=rng)
          imputed = donors
        else:
          imputed = y_pred_mis
        if cfg.range:
    ...
    for orig, (cols, cats) in self.onehot_groups_.items():
      ...
      for c in cats:
        oh_data.append(
          ...
          .otherwise(0)
          .alias(
            [c for c in cols if c.endswith(str(c)) or c.split("__")[-1] == str(c)][0]
            if cols
            else f"{orig}__{c}"
          )
        )
